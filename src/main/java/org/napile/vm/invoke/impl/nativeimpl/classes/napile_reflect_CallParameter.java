...
public class napile_reflect_CallParameter
{
	...
	private static BaseObjectInfo convertType(Vm vm, InterpreterContext context, TypeNode typeNode)
	{
		...
		BaseObjectInfo annotationArray = vm.newObject(context, NAPILE_LANG_ARRAY__ASM_ANNOTATION__, new TypeNode[]{AsmConstants.INT_TYPE}, new BaseObjectInfo[]{VmUtil.convertToVm(vm, context, 0)});
		...
		BaseObjectInfo parametersArray = vm.newObject(context, NAPILE_LANG_ARRAY__ASM_TYPE__, new TypeNode[]{AsmConstants.INT_TYPE}, new BaseObjectInfo[]{VmUtil.convertToVm(vm, context, typeNode.arguments.size())});
