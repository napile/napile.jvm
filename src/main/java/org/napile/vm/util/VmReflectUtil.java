...
{
	public static final FqName NAPILE_REFLECT_MODIFIER = new FqName("napile.reflect.Modifier");

	public static final TypeNode NAPILE_LANG_ARRAY__MODIFIER__ = new TypeNode(false, new ClassTypeNode(NapileLangPackage.ARRAY)).visitArgument(new TypeNode(false, new ClassTypeNode(NAPILE_REFLECT_MODIFIER)));
	...
	public static BaseObjectInfo createArray$Modifier$(Vm vm, InterpreterContext context, ReflectInfo memberNode)
	{
		...
		for(Modifier modifier : memberNode.getModifiers())
			value[i] = VmUtil.staticValue(vm, NAPILE_REFLECT_MODIFIER, modifier.name());
